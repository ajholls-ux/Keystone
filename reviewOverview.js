// ==========================================================================
// Decamark Field Kit -- Review Overview View
//
...
function formatVisitElapsed(ms) {
  if (ms < 0) ms = 0;
...
/**
 * Visit-level timer only (assessor aid). Does not affect scoring, reports, or completion.
 */
function renderVisitTimer(org, review, params) {
  const wrap = document.createElement("div");
  wrap.className = "visit-timer";

  const top = document.createElement("div");
  top.className = "visit-timer__row";

  const label = document.createElement("p");
  label.className = "visit-timer__label";
  label.textContent = "Visit timer";

  const value = document.createElement("p");
  value.className = "visit-timer__value";
  ...
  hint.className = "text-caption visit-timer__hint";

  if (!review.visitStartedAt) {
    value.textContent = "Not started";
    hint.textContent =
      "After you check in with the manager, start this when you begin observing. Assessor only — not on the client report.";
    wrap.append(hint);

    wrap.append(
      createButton({
        label: "Start visit timer",
        variant: "secondary",
        onClick: () => {
          updateState((s) => {
            const found = s.organisations
              .find((o) => o.id === org.id)
              ?.reviews.find((r) => r.id === review.id);
            if (!found || found.visitStartedAt) return s;
            found.visitStartedAt = new Date().toISOString();
            found.lastUpdatedAt = new Date().toISOString();
            return s;
          });
          navigate("reviewOverview", {
  ...
  hint.textContent = "Running since check-in / start of observation. Assessor only.";
  wrap.append(hint);

  const startedAt = new Date(review.visitStartedAt).getTime();

  function paint() {
    value.textContent = formatVisitElapsed(Date.now() - startedAt);
  }
  paint();
  const intervalId = window.setInterval(paint, 1000);

  // Best-effort cleanup when the node is detached on next navigation
...
export function renderReviewOverview(container, params) {
  ...
  screen.append(
    heading,
    renderVisitTimer(org, review, params),
    renderSummaryCard(org, review, activeCycle)
  );
