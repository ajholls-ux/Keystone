...
const STORAGE_KEY = "decamark_field_kit_state";
...
function loadFromStorage() {
  try {
    ...
    console.error("Decamark: failed to load persisted state, starting fresh.", err);
...
function persist() {
  try {
    ...
    console.error("Decamark: failed to persist state.", err);
  }
}
...
export function exportBackup() {
  ...
  a.href = url;
  a.download = `decamark-backup-${timestamp}.json`;
    ...
    reader.onload = () => {
      try {
        ...
        if (!parsed || !Array.isArray(parsed.organisations)) {
          throw new Error("File does not match Decamark backup format.");
        }
        state = parsed;
        persist();
        notify();
